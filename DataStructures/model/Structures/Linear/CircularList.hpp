...
//  Data Structures
//
//  Created by Cortez, Jairo on 2/22/18.
...
Type getFrontIndex(int index);
int getSize() const;
};
template <class Type>
CircularList<Type> :: CircularList()
{
    front_>setPrevevious(end);
    front->setNext(end);
    
    
    end->setPrevious(fronr);
    end->setNext(front);
}
template <class Type>
CircularList<Type> :: ~CircularList()
...
template <class Type>
voidCircularList> :: add(Type item)
{
...
template <class TYpe>
Type CircularList<Type> :: getFromIndex(index index)
{
    assert (index >= 0 && index < this->size);
...
Type CircularList<Type> :: remove(int index)
{
    ...
    removedNext->Previous(removedPrevious);
